...
- Forms use `react-hook-form` + `zodResolver` on the client; the same Zod schema is used by the server action — single source of truth in `schemas/`.
...
- Log unexpected errors with the requestId from middleware.
...
- Playwright after Phase 5 for the lead → appointment → dispute happy path.
